...
def build_active_problem(H_single_sp, best_params, num_qubits_single, active_ids, ng_budget, ansatz):
...
def create_isas(backend_name, backend, use_noise_model, full, Hs_active, optimization_level):
...
def get_backend(backend_name, use_noise_model, noise_model_options, resilience_level, seed, shots, tags):
    ...
    if backend_name == "Aer":
        ...
        prec = 0.0 if shots is None else 1.0 / np.sqrt(shots)
...
def run_vqe(i,
    ...
    full, Hs_active, optimizers_active, num_params = build_active_problem(H_single_sp, best_params, num_qubits_single, active_ids, ng_budget, ansatz)
    ...
    full_isa, Hs_active_isa = create_isas(backend_name, backend, use_noise_model, full, Hs_active, optimization_level)
    ...
    while iter_idx < max_iter and active_ids:
        ...
        if newly_converged_global:
            ...
            if log_enabled: logger.info("  shrinking active set, new active_ids: %s", active_ids)
        ...
        if not active_ids:
            if log_enabled: logger.info(f"All copies converged by iter {iter_idx}")
            break
    ...
    for cid in range(num_copies):
        if log_enabled: logger.info(f"Copy {cid}: best_E={best_E[cid]}, best_param={best_params[cid]}")
...
if __name__ == "__main__":
    ...
    potential = "AHO"
    cutoff = 2
    num_copies = 10   # total copies (optimizers) you want initially
    ...
    base_path = os.path.join(repo_path,r"SUSY\SUSY QM\Qiskit\Paralleltesting\Aer\NoiseModelTesting", "test", potential)
    ...
    with Pool(processes=num_procceses) as pool:
    ...
    energies = [e for res in results for e in res["energies"]]
    success = [e for res in results for e in res["converged"]]
    num_iters = [e for res in results for e in res["iters"]]
