...
def truncate_spo_by_weight(op, *, keep_ratio=0.999, min_keep= 0):
...
def append_split_operator_evolution(
    ...
    *,
    ...
    # Full-step double the half-step 
    ...
    # ---- Build T operator in k-basis (Z-only) with full-step scaling
    T_diag = (-(dt / 2.0)) * k2
...
if __name__ == "__main__":
    ...
    for potential in ["QHO","AHO","DW"]:
        for cutoff in [2,4,8,16,32,64,128,256,512,1024]:
            ...
            base_path = os.path.join(repo_path,r"SUSY\SUSY QM\Qiskit\SKQD\BasisTesting\Aer\Position+QFT+Lie", potential)
