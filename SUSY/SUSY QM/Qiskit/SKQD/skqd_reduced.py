...
def diagonal_to_z_sparsepauliop(diag_vals: np.ndarray, nb: int, *, atol: float = 1e-12,
                                     drop_identity: bool = False) -> SparsePauliOp:
    ...
    for s, c in enumerate(coeffs):
        ...
        if abs(c) <= atol:
            continue
        # Qiskit label: rightmost char is qubit 0
        chars = ["I"] * nb
        for q in range(nb):
            if (s >> q) & 1:
                chars[nb - 1 - q] = "Z"
...
def truncate_spo_by_weight(op: SparsePauliOp, *, keep_ratio: float = 0.999, min_keep: int = 0,
                           weight_power: float = 2.0) -> tuple[SparsePauliOp, dict]:
    ...
    if cum.size == 0:
        return op, {"m": 0, "n": 0, "keep_ratio": keep_ratio, "total_weight": 0.0, "truncated_weight": 0.0}

    m = int(np.searchsorted(cum, target, side="left") + 1)
    m = max(m, int(min_keep))
    m = min(m, int(coeffs.size))

    keep = order[:m]
    truncated_weight = float(total - cum[m - 1])
    ...
    info = {
        "m": int(m),
        "n": int(coeffs.size),
        "keep_frac_terms": float(m / coeffs.size),
...
def append_split_operator_evolution(qc, qf, qb, basis_info, t, n_steps):
    ...
    H_V_full = diagonal_to_z_sparsepauliop(V_bos, nb, drop_identity=True)
    H_V_trunc, _ = truncate_spo_by_weight(H_V_full, keep_ratio=0.999, min_keep=0)

    H_T_full = diagonal_to_z_sparsepauliop(Tphase, nb, drop_identity=True)
    H_T_trunc, _ = truncate_spo_by_weight(H_T_full, keep_ratio=0.999, min_keep=0)

    # Z_f ⊗ Wpp
    H_wpp_b = diagonal_to_z_sparsepauliop(Wpp, nb, atol=1e-12, drop_identity=False) 
    H_wpp_b_trunc, _ = truncate_spo_by_weight(H_wpp_b, keep_ratio=0.999, min_keep=0)
    labels = ["Z" + p.to_label() for p in H_wpp_b_trunc.paulis]
    H_wpp = SparsePauliOp(labels, H_wpp_b_trunc.coeffs)

    evo_wpp = PauliEvolutionGate(H_wpp, time=(dt / 4.0), synthesis=LieTrotter(reps=1))
    evo_V = PauliEvolutionGate(H_V_trunc, time=-1.0, synthesis=LieTrotter(reps=1))
    evo_T = PauliEvolutionGate(H_T_trunc, time=-1.0, synthesis=LieTrotter(reps=1))
