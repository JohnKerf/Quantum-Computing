...
def append_split_operator_evolution(
    ...
    *,
    ...
    """
    Split-operator evolution using *direct diagonal unitaries* via DiagonalGate.

    This replaces Walsh->Z Pauli expansions for diagonal operators.

    Notes:
      - q-basis (x-basis): Wp2 and Wpp are diagonal.
      - k-basis: k2 is diagonal, so we apply QFT -> DiagonalGate(k2 phases) -> iQFT.
      - bf term: implements exp(-i * time * (Z_f ⊗ diag(Wpp)) ) as a single diagonal on [qf]+qb.
    """
    ...
    # Optional: zero out tiny values to reduce numerical noise in phases (won't change 2Q count)
    def _clean(v):
        v = v.copy()
        v[np.abs(v) < atol] = 0.0
        return v
    ...
    # ----------------------------
    # Build diagonal gates
    # ----------------------------
    # Your old code used:
    #   V_bos_half_diag = (-(dt/4)) * Wp2
    # and then applied exp(-i * V_bos_half_diag) via PauliEvolutionGate(time=1).
    V_bos_half_diag = (-(dt / 4.0)) * Wp2
    V_bos_full_diag = 2.0 * V_bos_half_diag

    gate_V_half = DiagonalGate(np.exp(-1j * V_bos_half_diag))
    gate_V_full = DiagonalGate(np.exp(-1j * V_bos_full_diag))

    # Kinetic in k-basis:
    #   T_diag = (-(dt/2)) * k2  -> apply exp(-i * T_diag) in Fourier basis
    T_diag = (-(dt / 2.0)) * k2
    gate_T = DiagonalGate(np.exp(-1j * T_diag))

    # bf term: exp(-i * time * (Z_f ⊗ diag(Wpp)))
    #
    # For fermion qubit:
    #   |0> -> Z = +1  => phases = exp(-i * time * (+Wpp))
    #   |1> -> Z = -1  => phases = exp(-i * time * (-Wpp)) = exp(+i * time * Wpp)
    #
    # We'll build a diagonal of length 2**(nb+1) for qubits [qf] + qb.
    def bf_diag_phases(time_factor: float) -> np.ndarray:
        phases_f0 = np.exp(-1j * time_factor * (+Wpp))
        phases_f1 = np.exp(-1j * time_factor * (-Wpp))
        return np.concatenate([phases_f0, phases_f1])

    # Your old PauliEvolutionGate used time dt/4 for half-step and dt/2 for full-step
    gate_bf_half = DiagonalGate(bf_diag_phases(dt / 4.0))
    gate_bf_full = DiagonalGate(bf_diag_phases(dt / 2.0))
    ...
    def apply_V(kind: str):
        if kind == "half":
            qc.append(gate_V_half, qb)
            qc.append(gate_bf_half, [qf] + qb)
        elif kind == "full":
            qc.append(gate_V_full, qb)
            qc.append(gate_bf_full, [qf] + qb)
        else:
...
if __name__ == "__main__":
    ...
    for potential in ["QHO","AHO","DW"]:
        for cutoff in [2,4,8,16,32,64,128,256, 512,1024]:
            ...
            base_path = os.path.join(repo_path,r"SUSY\SUSY QM\Qiskit\SKQD\BasisTesting\RealTranspile\Test5", potential)
