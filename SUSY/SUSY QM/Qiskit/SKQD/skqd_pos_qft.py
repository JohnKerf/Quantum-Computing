...
def summarize_circuit(qc, name=""):
    ops = qc.count_ops()
    n2 = sum(1 for inst in qc.data if inst.operation.num_qubits == 2 and inst.operation.name not in {"barrier","measure"})
    print(f"\n=== {name} ===")
    print("depth:", qc.depth(), " size:", qc.size(), " 2Q:", n2)
    print("swap:", ops.get("swap", 0))
    for g in ["cz","cx","ecr","crz","cp","rzz"]:
        if ops.get(g, 0):
            print(f"{g}: {ops[g]}")
    print("top ops:", dict(sorted(((k,int(v)) for k,v in ops.items()), key=lambda kv: kv[1], reverse=True)[:12]))

from qiskit.transpiler.layout import Layout

def print_layout_info(tqc):
    layout = getattr(tqc, "_layout", None)
    print("\n=== Layout info ===")
    if layout is None:
        print("No _layout attached to circuit.")
        return
    try:
        il = layout.initial_layout
        fl = layout.final_layout
        print("initial_layout:", il)
        print("final_layout  :", fl)
    except Exception as e:
        print("Could not read layout fields:", e)
...
def append_split_operator_evolution(qc, qf, qb, basis_info, t, n_steps):
    ...
    # Potential half-step:
    # V = 1/2 * (Wp2 + Z*Wpp)
    # exp[-i (dt/2) * V] = exp[i * (-dt/4) * (Wp2 + Z*Wpp)]

    # boson diagonal: exp[i * (-dt/4) * Wp2]
    V_wp2 = (-dt / 4.0) * Wp2
    gate_V_wp2 = DiagonalGate(np.exp(1j * V_wp2).tolist())

    # Z*Wpp with controlled gate:
    V_Wpp_pos = (-dt / 4.0) * (+Wpp)
    gate_V_Wpp_pos = DiagonalGate(np.exp(1j * V_Wpp_pos).tolist())

    Wpp_ctrl1 = (dt / 2.0) * Wpp
    gate_Wpp_ctrl1 = DiagonalGate(np.exp(1j * Wpp_ctrl1).tolist()).control(1, ctrl_state="1")

    # Kinetic full-step:
    # T = 1/2 * p^2
    # exp[-i dt * T] = exp[i * (-(dt/2) * k2)]
    T = (-(dt / 2.0)) * k2
    ...
    for _ in range(n_steps):
        # V half-step
        qc.append(gate_V_wp2, qb)
        qc.append(gate_V_Wpp_pos, qb)
        qc.append(gate_Wpp_ctrl1, [qf] + qb)
...
if __name__ == "__main__":
    ...
    for potential in ['DW']:
        for cutoff in [2,4,8,16,32,64,128,256,512,1024]:
            ...
            base_path = os.path.join(repo_path,r"SUSY\SUSY QM\Qiskit\SKQD\BasisTesting\RealTranspile\Position+QFT", potential)
