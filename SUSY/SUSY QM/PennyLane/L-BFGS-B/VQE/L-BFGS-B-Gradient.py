...
def cost_function(params, H, num_qubits, shots):
    ...
    return energy, device_time, gradient
...
def run_vqe(i, max_iter, tol, H, num_qubits, shots):
    ...
    x0 = qml.numpy.array(np.random.random(size=2*num_qubits) * 2 * np.pi, requires_grad=True)

    device_time = timedelta()

    def wrapped_cost_function(params):
        result, dt, _ = cost_function(params, H, num_qubits, shots)
        nonlocal device_time
        device_time += dt
        return result
    
    def wrapped_grad_function(params):
        _, _, grad = cost_function(params, H, num_qubits, shots)
        return grad
...
if __name__ == "__main__":
    ...
    for potential in potential_list:
        ...
        base_path = os.path.join(r"C:\Users\johnkerf\Desktop\Quantum-Computing\Quantum-Computing\SUSY\SUSY QM\PennyLane\VQE\OptimizerComparison\L-BFGS-B\gradtest", potential, str(shots))
