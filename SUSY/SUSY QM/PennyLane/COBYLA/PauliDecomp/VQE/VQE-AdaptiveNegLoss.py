...
def cost_function(params, paulis, coeffs, num_qubits, shots, dev, seed):
...
def run_vqe(i, max_iter, tol, paulis, coeffs, num_qubits, shots, num_params, device, log_dir, log_enabled, eps, lam, p):
    ...
    def wrapped_cost_function(params):
        ...
        result = cost_function(params, paulis, coeffs, num_qubits, shots, dev, seed)
    ...
    def callback(xk, convergence=None):
        if log_enabled and iteration_count['k'] % 10 == 0:
            logger.info(f"Iteration {iteration_count['k']}: Energy = {last_energy:.8f}")
...
if __name__ == "__main__":
    ...
    shotslist = [None] #No bounds when None
    cutoffs = [16]#,32,64]
    ...
    for shots in shotslist:
        for cutoff in cutoffs:
            ...
            base_path = os.path.join("/users/johnkerf/fastscratch/Data/PennyLane/COBYLA/PauliDecomp/VQE/Real-Amplitudes", str(shots), potential)
            ...
            with Pool(processes=100) as pool:
                vqe_results = pool.starmap(
                    run_vqe,
                    [
                        (i, max_iter, tol, paulis, coeffs, num_qubits, shots, num_params, device, log_path, log_enabled, eps, lam, p)
