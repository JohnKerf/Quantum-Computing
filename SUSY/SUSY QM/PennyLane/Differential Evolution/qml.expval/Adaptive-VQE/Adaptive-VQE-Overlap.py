...
def compute_overlap(params, num_qubits, op_list, basis_state, min_eigenvector):

    dev1 = qml.device("default.qubit", wires=num_qubits, shots=None)
    @qml.qnode(dev1)
    def overlap_circuit(params, op_list):
    ...
    state = overlap_circuit(params, op_list)
    overlap = np.vdot(min_eigenvector, state)
...
def run_adapt_vqe(i, max_iter, tol, H, num_qubits, shots, num_steps, phi, num_grad_checks, operator_pool, basis_state, min_eigenvalue, min_eigenvector, log_dir, strategy, abs_tol, popsize):
    ...
    for i in range(num_steps):
        ...
        if i != 0:
            ...
            if (type(most_common_gate) == qml.SingleExcitation) or (type(most_common_gate) == qml.CRY):
                ...
                if (qml.RX(phi, wires=cq) not in pool):
                    pool.append(qml.RX(phi, wires=cq))
                ...
                if (qml.RX(phi, wires=tq) not in pool):
                    pool.append(qml.RX(phi, wires=tq))
        ...
        update = True
        overlap = compute_overlap(res.x, num_qubits, op_list, basis_state, min_eigenvector)

        if overlap > previous_overlap:
            logger.info(f"Adding {most_common_gate} increased overlap from {previous_overlap} to {overlap}")
            previous_overlap = overlap    
        elif np.abs(overlap - previous_overlap) < 1e-2: 
            logger.info(f"Adding {most_common_gate} had small change in overlap from {previous_overlap} to {overlap}")
            previous_overlap = overlap
        else:
            logger.info(f"Adding {most_common_gate} decreased overlap from {previous_overlap} to {overlap}.. skipping this gate")
            #logger.info(f"previous gate list: {op_list}")
            op_list = op_list[:-1]
            #logger.info(f"updated gate list: {op_list}")
            update = False
        ...
        if i!=0:
            if abs(pre_min_e - min_e) < 1e-8:
                logger.info("gradient converged")
                if update:
                    #logger.info(op_list)
                    energies.pop()
                    op_list.pop()
            ...
            if abs(min_eigenvalue-min_e) < 1e-6:
                logger.info(f"Converged to min eigenvalue: {min_eigenvalue}")
                success = True
                final_params = op_params
                break
    ...
    logger.info(f"Completed VQE run {i}")
    logger.info(f"Final ops list: {final_ops}")
    logger.info(f"Energy: {min_e}")
...
if __name__ == "__main__":
    ...
    for cutoff in cutoff_list:
        ...
        base_path = os.path.join(r"/users/johnkerf/Quantum Computing/SUSY-QM/PennyLane/ADAPT-VQE/DifferentialEvolution/StatevectorFiles-Overlap", potential)
        ...
        eiv, eigenvectors = np.linalg.eig(H)
        min_3_ev = eiv.argsort()[:4]
        min_eigenvector = np.asarray(eigenvectors[:, min_3_ev[0]])
        min_eigenvalue = min_3_ev[0]
        ...
        with Pool(processes=100) as pool:
            vqe_results = pool.starmap(
                run_adapt_vqe,
                [
                    (i, max_iter, tol, H, num_qubits, shots, num_steps, phi, num_grad_checks, operator_pool, basis_state, min_eigenvalue, min_eigenvector, log_path, strategy, abs_tol, popsize)
        ...
        all_overlaps = [res["overlaps"] for res in vqe_results]
