...
def run_adapt_vqe(i, max_iter, tol, H, num_qubits, shots, num_steps, phi, num_grad_checks, operator_pool, basis_state, min_eigenvalue, min_eigenvector):
    ...
    for i in range(num_steps):
        ...
        if overlap > previous_overlap:
            print(f"Adding {most_common_gate} increased overlap from {previous_overlap} to {overlap}")
            previous_overlap = overlap
        elif np.abs(overlap - previous_overlap) < 1e-2: 
            print(f"Adding {most_common_gate} had small change in overlap from {previous_overlap} to {overlap}")
            previous_overlap = overlap
        else:
            print(f"Adding {most_common_gate} decreased overlap from {previous_overlap} to {overlap}.. skipping this gate")
            #print(f"previous gate list: {op_list}")
            op_list = op_list[:-1]
            #print(f"updated gate list: {op_list}")
...
if __name__ == "__main__":
    ...
    for cutoff in cutoff_list:
        ...
        base_path = os.path.join(r"C:\Users\Johnk\Documents\PhD\Quantum Computing Code\Quantum-Computing\SUSY\SUSY QM\PennyLane\ADAPT-VQE\COBYLA-Test\StatevectorFiles-Overlap", potential)
        ...
        with Pool(processes=8) as pool:
            vqe_results = pool.starmap(
                run_adapt_vqe,
                [
                    (i, max_iter, tol, H, num_qubits, shots, num_steps, phi, num_grad_checks, operator_pool, basis_state, min_eigenvalue, min_eigenvector)
