...
def run_vqe(i, paulis, coeffs, ansatz, run_info, log_enabled, log_dir):
    ...
    def wrapped_cost_function(params):
        """Objective passed to Nevergrad (params: np.ndarray)."""
    ...
    # === NGOpt + early stopping parameters ===
    max_iter = run_info["optimizer_options"]["maxiter"]
    tol = run_info["optimizer_options"].get("tol", 0.0)
    ...
    with qml.Tracker(dev) as tracker:
        for k in range(max_iter):
            ...
            if best_obj is None or obj < best_obj:
                ...
                # no improvement in best objective
                if tol > 0:
                    no_improve_rounds += 1
    ...
    # Fall back in case best_obj never got set (shouldn't happen if max_iter>0)
    if best_obj is None:
        best_params = x0
    ...
    results_data = {
        "seed": seed,
        # store both penalized objective and raw energy
        "energy": final_obj,
