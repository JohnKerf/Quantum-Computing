...
def cost_function(params, prev_param_list, H_decomp, num_qubits, beta, dev, swap_dev):
...
def run_vqd(i, max_iter, tol, initial_tr_radius, final_tr_radius, H_decomp, num_qubits, num_params, shots, swap_shots, num_energy_levels, beta):
    ...
    swap_dev = qml.device("default.qubit", wires=2*num_qubits, shots=swap_shots, seed=seed)
    ...
    for _ in range(num_energy_levels):
        ...
        with Tdev, Tswap:
            res = minimize(
                cost_function,
                x0,
                args=(prev_param_list, H_decomp, num_qubits, beta, dev, swap_dev),
                #bounds=bounds,
                #tol=tol,
                method= "COBYQA",
                options= {
...
if __name__ == "__main__":
    ...
    for swap_shots in swap_shots_list:

        print(f"Running for {str(swap_shots)} swap_shots")
            ...
            for cut_off in cut_offs_list:
                ...
                with Pool(processes=10) as pool:
                    vqd_results = pool.starmap(
                        run_vqd,
                        [
                            (i, max_iter, tol, initial_tr_radius, final_tr_radius, H_decomp, num_qubits, num_params, shots, swap_shots, num_energy_levels, beta)
