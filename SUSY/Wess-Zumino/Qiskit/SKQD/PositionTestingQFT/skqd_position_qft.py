...
def create_circuit(backend, basis_info, basis_state, avqe_circuit, optimization_level, num_qubits, H_pauli, t_k, num_trotter_steps, boundary_condition, keep_ratio):
    ...
    wz.append_wz_split_operator_evolution(qc, qf_sites, qb_sites, basis_info, dt_step, num_trotter_steps, boundary_condition=boundary_condition,
                                            keep_ratio_1site = keep_ratio,
                                            keep_ratio_2site = keep_ratio,
                                            min_keep_1site = 0,
                                            min_keep_2site = 0)

    #evol_gate = PauliEvolutionGate(H_pauli,time=dt_step,synthesis=LieTrotter(reps=num_trotter_steps))
...
def truncate_by_coeff_weight(pauli_coeffs, pauli_labels, keep_ratio=0.999, min_keep=0, restore_original_order=True):
    ...
    keep_idx = order[:m].astype(int)     

    keep_idx_sorted = np.sort(keep_idx) if restore_original_order else keep_idx

    truncated = float(total - (cum[m-1] if m > 0 else 0.0))

    info = {
        "m": m,
        "n": len(c),
        "keep_frac_terms": (m / len(c)) if len(c) else 0.0,
    ...
    return c[keep_idx_sorted], lab[keep_idx_sorted], info


def run_skqd(run_idx, parts, H_pauli, H_info, Ansatz_info, pauli_terms, avqe_circuit, backend_info, run_info, base_path, log_enabled):
    ...
    # --- control the trotter step size as k grows ---
    dt_step_max = run_info.get("dt_step_max", 2.0)  # tune this

    while not converged and k <= max_k:

        if log_enabled: logger.info(f"Running for Krylov dimension {k}")
        print(f"Run {run_idx} step {k}")

        t_k = dt*k
        
        n_steps = int(np.ceil(t_k / dt_step_max))
        n_steps = max(1, min(n_steps, max_n_steps))

        
        qc, circuit_cost = create_circuit(backend, parts, H_info["basis"], avqe_circuit, backend_info["optimization_level"], num_qubits, H_pauli, t_k, n_steps, H_info["boundary_condition"], keep_ratio)
...
if __name__ == "__main__":
    ...
    for cutoff in [16]:
        ...
        base_path = os.path.join(r"C:\Users\Johnk\Documents\PhD\Quantum Computing Code\Quantum-Computing\SUSY\Wess-Zumino\Qiskit\SKQD\PositionTestingTrunc", boundary_condition, potential, folder)
        ...
        #H_path = os.path.join(r"C:\Users\Johnk\Documents\PhD\Quantum Computing Code\Quantum-Computing\SUSY\Wess-Zumino\Analyses\Model Checks\HamiltonianDataPosition", boundary_condition, potential, folder, f"{potential}_{cutoff}.json")
        try:
            H_path = os.path.join(r"C:\Users\Johnk\Documents\PhD\Quantum Computing Code\Quantum-Computing\SUSY\Wess-Zumino\Analyses\x_max_sweep\HamiltonianDataPosition-XMax", boundary_condition, potential, folder, str(x_max), f"{potential}_{cutoff}.json")
            with open(H_path, 'r') as file:
                H_data = json.load(file)

            eigenvalues = H_data['eigenvalues']
        except:
            print("File not found setting eigenvalues to None")
            eigenvalues = None

        H_pauli, n_qubits, parts = wz.build_wz_hamiltonian_with_qft_parts(cutoff=cutoff, N=N, a=a, potential=potential, boundary_condition=boundary_condition,c=c, x_max=x_max)
        pauli_labels = H_pauli.paulis
        pauli_coeffs = H_pauli.coeffs
        pauli_terms = [(np.real(c), p.to_label()) for c, p in zip(H_pauli.coeffs, H_pauli.paulis)]

        kept_coeffs, kept_labels, trunc_info = truncate_by_coeff_weight(np.real(pauli_coeffs), [p.to_label() for p in pauli_labels], keep_ratio=keep_ratio)
        ...
        num_qubits = int(N*(1+np.log2(cutoff)))
        ...
        run_skqd(0, parts, H_pauli, H_info, Ansatz_info, pauli_terms, avqe_circuit, backend_info, run_info, base_path, log_enabled)
