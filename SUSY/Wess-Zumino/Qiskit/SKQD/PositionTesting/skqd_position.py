...
def create_circuit(backend, basis_state, avqe_circuit, optimization_level, num_qubits, H_pauli, t_k, num_trotter_steps):
    ...
    dt = t_k / num_trotter_steps

    evol_gate = PauliEvolutionGate(H_pauli,time=dt,synthesis=LieTrotter(reps=num_trotter_steps))
...
def run_skqd(run_idx, parts, H_pauli, H_info, Ansatz_info, pauli_terms, avqe_circuit, backend_info, run_info, base_path, log_enabled):
    ...
    while not converged and k <= max_k:
        ...
        qc, circuit_cost = create_circuit(backend, H_info["basis"], avqe_circuit, backend_info["optimization_level"], num_qubits, H_pauli, t_k, n_steps)
