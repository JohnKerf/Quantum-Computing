...
def run_adapt_vqe(run_idx, H_pauli, run_info):
    ...
    eps_screen = run_info['eps_screen']
    ...
    # -----------------------------
    # grad_conv_thresh = run_info.get("grad_conv_thresh", 1e-8)
    # grad_conv_patience = run_info.get("grad_conv_patience", 3)  # consecutive iterations below threshold
    # min_iters_before_grad_conv = run_info.get("min_iters_before_grad_conv", 10)
    # _below_count = 0
    ...
    for i in range(run_info["num_steps"]):
        ...
        # if abs(min_e - run_info["min_eigenvalue"]) < energy_conv_tol:
        #     print(f"Energy converged: |E - E0| = {abs(min_e - run_info['min_eigenvalue']):.3e}")
        #     success = True
        ...
        if i >=5:
            if abs(pre_min_e - min_e) < energy_conv_tol:
                patience_count += 1
        ...
        if abs(run_info["min_eigenvalue"] - min_e) < energy_conv_tol:
