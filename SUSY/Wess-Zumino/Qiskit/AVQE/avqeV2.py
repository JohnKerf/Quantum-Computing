...
def finite_diff_grad(estimator, circuit, observable, x, pos, eps=1e-4):
    """
    Finite-difference d/dx_k <H> at point x using 2 evaluations.
    """
...
def grad_for_candidate_op(
    ...
    *,
    ...
    ops.insert(pos, candidate_op) #insert candidate op at pos index
    ...
    arr = np.asarray(selected_params, dtype=float)
    if arr.size == 0:
        x = np.array([param0], dtype=float)
    else:
        x = np.insert(arr, pos, float(param0))

    # Full gradient; we only need component for the NEW (last) parameter
    g = finite_diff_grad(estimator, qc, H_pauli, x, pos, eps=fd_eps)
    return g
...
def run_adapt_vqe(run_idx, H_pauli, run_info):
    ...
    grad_scan_after = run_info.get("grad_scan_after", 3)  # e.g. 10
    ...
    for i in range(run_info["num_steps"]):
        ...
        if i < grad_scan_after or Op_L == 0:
            
            print(f"Run {i}: computing gradient - appending")
            ...
            insert_pos = Op_L
            chosen_op = most_common_gate
            #op_list.append(most_common_gate)

        else:
            print(f"Run {i}: computing gradient (ALL ops x ALL positions)")
            # choose how you want to seed the new parameter during the scan:
            # param0 = 0.0 is common; or keep a small random value
            param0 = 0.0
            ...
            print(f"Run {i}: best |grad|={max_grad:.3e} with {chosen_op.name} on {chosen_op.qubits} at pos={insert_pos}")

        op_list.insert(insert_pos, chosen_op)
        op_params = np.insert(np.asarray(op_params, dtype=float), insert_pos, 0.0).tolist()

        print(op_list, op_params)

        print(f"Run {i}: selected {chosen_op.name} on {chosen_op.qubits} at pos={insert_pos}")
