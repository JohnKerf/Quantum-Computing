...
def clamp_and_renorm(weights: dict[str, float]) -> dict[str, float]:
    """For basis selection: clamp negative weights to 0 and renormalize to sum=1."""
    if not weights:
        return {}
    w2 = {s: max(0.0, float(w)) for s, w in weights.items()}
    z = sum(w2.values())
    if z == 0.0:
        return {}
    return {s: w / z for s, w in w2.items()}

def filter_weights_by_fermion_number(
    ...
    *,
    ...
    for key, w in weights.items():
        b = [int(ch) for ch in key[::-1]]  # matches your current convention
        occ = sum(b[i] for i in fermion_qubits)
        if abs(occ - num_fermions) <= tol:
            kept[key] = w
        else:
            rejected += w
    return kept, rejected

def trim_weights(weights: dict[str, float], p_keep: float) -> dict[str, float]:
    """Keep the largest-weight states until cumulative mass >= p_keep."""
    if not weights:
        return {}
...
if __name__ == "__main__":
    ...
    TRIM_STATES = True
    P_KEEP = 0.995
    USE_QUASI_WEIGHTS = True      # use floats (probs/quasi) for basis selection
    MAX_GLOBAL_STATES = 4000      # optional hard cap so samples doesn't blow up
    ...
    mit = None
    if MITIGATE_READOUT:   # you decide this flag
        mit = mthree.M3Mitigation(backend)
        mit.cals_from_system(qubits=list(range(num_qubits)))
    ...
    avqe_circuit = wz.build_avqe_pattern_ansatz(N=N, cutoff=cutoff, include_basis=True)
    ...
    while not converged and k <= max_k:
        ...
        # ---- counts -> weights (probabilities / quasi) ----
        raw_counts = counts
        raw_unique = len(raw_counts)
        raw_shots = sum(raw_counts.values())

        if USE_QUASI_WEIGHTS:
            if mit is not None:
                # counts -> mitigated quasi distribution
                quasi = mit.apply_correction(raw_counts, qubits=list(range(num_qubits)))

                # mthree may return a dict-like or a QuasiDistribution; force to plain dict
                quasi_dict = dict(quasi)

                # for basis selection: clamp negatives and renormalize
                weights = clamp_and_renorm(quasi_dict)
            else:
                # fallback: just probabilities from raw counts
                weights = counts_to_probs(raw_counts)
        else:
            # Old behaviour: integer counts
            weights = dict(raw_counts)
        ...
        if CONSERVE_FERMIONS:
            if USE_QUASI_WEIGHTS:
                weights, rej_w = filter_weights_by_fermion_number(
                    weights,
                ...
                post_rejected += rej_w
                weights = clamp_and_renorm(weights)  # keep weights well-behaved
                print(f"Rejected weight {rej_w:.6f} from conserving fermion number")
            else:
                ...
                post_rejected += rej_w
                print(f"Rejected {rej_w} shots from conserving fermion number")
                weights = dict(counts)  # keep downstream code uniform

        # ---- trim within step (strongly recommended on hardware) ----
        if TRIM_STATES:
            if USE_QUASI_WEIGHTS:
                pre = sum(weights.values())
                weights = trim_weights(weights, P_KEEP)
                post = sum(weights.values())
                trim_rejected = pre - post
                weights = clamp_and_renorm(weights)
                print(f"Trimmed weight {trim_rejected:.6f} with p_keep={P_KEEP}")
            else:
                pre_trim_shots = sum(weights.values())
                weights = trim_counts(weights, P_KEEP)
                trim_rejected = pre_trim_shots - sum(weights.values())
                print(f"Trimmed {trim_rejected} shots with p_keep={P_KEEP}")

        kept_unique = len(weights)
        kept_mass = float(sum(weights.values())) if USE_QUASI_WEIGHTS else None
        kept_shots = sum(weights.values()) if not USE_QUASI_WEIGHTS else int(round(kept_mass * raw_shots))
        ...
        if log_enabled:
            logger.info(json.dumps({
                ...
                "raw_shots": raw_shots,
                "postselect_rejected": post_rejected,
                "trim_rejected": trim_rejected,
                "kept_unique": kept_unique,
                "kept_shots_equiv": kept_shots,
                "kept_mass": kept_mass,
            }, indent=4, default=str))

        # ---- Update global samples ----
        # Counter.update works with floats too, so this is a clean drop-in.
        samples.update(weights)

        # Optional hard cap to stop basis exploding:
        if MAX_GLOBAL_STATES is not None and len(samples) > MAX_GLOBAL_STATES:
            samples = Counter(dict(samples.most_common(MAX_GLOBAL_STATES)))
